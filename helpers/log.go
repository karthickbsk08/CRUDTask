...
func (h *HelperStruct) SetReference(pReference any) {
...
func (h *HelperStruct) Log(pDebugLevel int, pMsg ...any) {
	...
	lLevel := fmt.Sprintf("%v", lConfigFile.(map[string]any)["LogCategory"])
	...
	lBase64FileLevel := fmt.Sprintf("%v", lConfigFile.(map[string]any)["Base64Encode"])
	...
	lReference := fmt.Sprintf("%v", lConfigFile.(map[string]any)["LogReference"])
...
func ReplaceBase64String(pBase64String string, pFilterCategory int) string {
	...
	lConfigFile := tomlutil.ReadTomlConfig("./toml/debug.toml")
	lThreshold := fmt.Sprintf("%v", lConfigFile.(map[string]any)["Threshold"])
