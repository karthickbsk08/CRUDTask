...
func ApiCallLogCapture(count int, pRequest ApiLogCapture) {
	pRequest.PDebug.Log(helpers.Statement, "ApiCallLogCapture (+)", count, connectdb.GDB.GPostgres.Stats())
	...
	lResult := connectdb.GDB.GRMPostgres.Table("xxapi_req_log").Create(&pRequest)
